...
// Renderable types
#define RENDERABLE_TYPE_STATIC 0 // Geometry that usually doesn't move and can extend to very large scales, typically Terrain
#define RENDERABLE_TYPE_DYNAMIC 1 // Highly dynamic, small objects, typically mobs and temporary-animated objects, they do not occlude indirect lighting
#define RENDERABLE_TYPE_SELF 2 // Typically refers to the current player's avatar when in first person view, invisible for primary rays but visible for reflections and shadow rays
#define RENDERABLE_TYPE_VOLUME 3 // Geometry that is transparent and that can include other geometries within it, typically Water, may be somewhat dynamic
#define RENDERABLE_TYPE_SPECIAL_1 4 // Reserved for custom renderer-specific things
#define RENDERABLE_TYPE_SPECIAL_2 5 // Reserved for custom renderer-specific things
...
#define RENDERABLE_MASK_SPECIAL_1 (1u<< RENDERABLE_TYPE_SPECIAL_1)
#define RENDERABLE_MASK_SPECIAL_2 (1u<< RENDERABLE_TYPE_SPECIAL_2)
...
#define RENDERABLE_STANDARD (0xff & ~(RENDERABLE_MASK_SPECIAL_1 | RENDERABLE_MASK_SPECIAL_2 | RENDERABLE_MASK_LIGHT_RADIUS))
#define RENDERABLE_PRIMARY (RENDERABLE_STANDARD & ~RENDERABLE_MASK_SELF)
#define RENDERABLE_STANDARD_EXCEPT_VOLUME (RENDERABLE_STANDARD & ~RENDERABLE_MASK_VOLUME)
...
#define RENDERABLE_TYPE_ANTI_WATER RENDERABLE_TYPE_SPECIAL_1
#define RENDERABLE_MASK_ANTI_WATER RENDERABLE_MASK_SPECIAL_1
...
#define MAX_WATER_DEPTH renderer.waterMaxLightDepth
	...
	void ApplyFog(in vec3 origin, in vec3 dir) {
		if ((RT_PAYLOAD_FLAGS & RT_PAYLOAD_FLAG_UNDERWATER) == 0 && ray.hitDistance > 0) {
			const float distFactor = min(1, ray.hitDistance / FOG_DISTANCE_MAX);
			const float fogStrength = pow(distFactor, 1.5);
			const vec3 fogColor = mix(renderer.skyLightColor, GetSkyColor(dir), distFactor);
			if (ray.bounces == 0 && renderer.fogSteps > 0) {
				...
				for (int i = 0; i < renderer.fogSteps; ++i) {
					...
					ray.hitDistance = 0;
					traceRayEXT(tlas, 0, RENDERABLE_PRIMARY_EXCEPT_VOLUME, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, worldPos, 0, normalize(renderer.sunDir + vec3(0.0012765f)), camera.zFar, RAY_PAYLOAD_PRIMARY);
	...
	vec3 GetDirectLighting(in vec3 albedo, in float diffuseMultiplier, in float specularMultiplier, in float specularPower) {
		...
		bool isUnderWater = (RT_PAYLOAD_FLAGS & RT_PAYLOAD_FLAG_UNDERWATER) != 0;
			...
			if (ray.bounces < 2) {// Direct Lighting (must apply this for diffuse materials only)
				...
				if (dot(shadowRayDir, ray.normal) > 0.001) {
					...
					for (;;) {
						...
						ray.hitDistance = 0;
						++ray.bounces;
						traceRayEXT(tlas, 0, RENDERABLE_STANDARD_EXCEPT_VOLUME, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, surfacePosition, camera.zNear, shadowRayDir, camera.zFar, RAY_PAYLOAD_PRIMARY);
						if (ray.hitDistance == -1) {
							vec3 diffuse = albedo * renderer.skyLightColor * dot(renderer.sunDir, originalRay.normal) * diffuseMultiplier;
							vec3 specular = renderer.skyLightColor * pow(clamp(dot(originalRay.normal, normalize(shadowRayDir - gl_WorldRayDirectionEXT)), 0, 1), specularPower) * specularMultiplier;
...
#line 12 "/home/olivier/projects/chill/src/v4d/modules/V4D_rtcubes/assets/shaders/avatar.glsl"
