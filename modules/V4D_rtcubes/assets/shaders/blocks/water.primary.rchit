			...
			if (BOX_FACE != 1 && gl_HitKindEXT == BOX_INTERSECTION_KIND_OUTSIDE_FACE && waterDepth == 0) {
				...
				if (dot(gl_WorldRayDirectionEXT, vec3(0,1,0)) > 0) {
					...
					if (isFirstRay) {
						...
						// Launch ray to underwater objects (should exclude water blocks)
						ray.hitDistance = 0;
						traceRayEXT(tlas, 0, RENDERABLE_PRIMARY_EXCEPT_WATER, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, rayPosition, camera.zNear, rayDirection, MAX_WATER_DEPTH, RAY_PAYLOAD_PRIMARY);
						if (ray.hitDistance == -1) {
							ray = waterRay;
