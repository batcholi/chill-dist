...
void main() {
	CLOSEST_HIT_BEGIN
		...
		ray.color = vec4(0.8, 0.8, 1.1, 0.5);
		
		ApplyFresnelReflection(1.3);
		
		vec3 albedo = ray.color.rgb;
		ApplyAmbientBlockLighting();
		ApplyDirectLighting(albedo, 1.0/*diffuse*/, 0.5/*specular*/, 5.0/*specularPow*/);
	...
	if (Refract(rayDirection, normalize(ray.normal), 1.3)) {
		...
		ray.nextPosition += rayDirection * (t2 - camera.zNear * 2);
		ray.color.a = max(0.5, pow(clamp(t2, 0, 1), 0.5));
	} else {
		ray.color.a = 1;
	}
