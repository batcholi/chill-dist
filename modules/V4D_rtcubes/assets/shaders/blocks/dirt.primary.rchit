...
void main() {
	CLOSEST_HIT_BEGIN
		...
		// Dirt texture
		ray.color = ApplyTexture(1, BOX_UV);
		
		// Grass texture
		if (GetOpaqueBlockExtra(GetBlockData()) == 1) {
			if (BOX_FACE == 4) {
				// Top
				vec4 grass = ApplyTexture(3, BOX_UV) * vec4(0.4, 0.5, 0.25, 1.0);
				ray.color = mix(ray.color, grass, grass.a);
			} else if (BOX_FACE != 1) {
				// Sides
				vec4 grass = ApplyTexture(2, BOX_UV) * vec4(0.4, 0.5, 0.25, 1.0);
				ray.color = mix(ray.color, grass, grass.a);
			}
		}
