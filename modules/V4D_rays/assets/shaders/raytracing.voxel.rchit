...
#line 455 "/home/olivier/projects/chill/src/v4d/modules/V4D_rays/assets/shaders/raytracing.glsl"
...
vec3 GetAmbientLighting(in uint giIndex, in vec3 worldPosition, in vec3 posInVoxel, in vec3 normal) {
	if (abs(GetGi(giIndex).frameIndex - int64_t(camera.frameIndex)) < ACCUMULATOR_MAX_FRAME_INDEX_DIFF && GetGi(giIndex).radiance.a > 1) {
		vec4 lighting = vec4(GetGi(giIndex).radiance.rgb, 1);
		for (int i = 0; i < nbAdjacentSides; ++i) {
			if (abs(dot(vec3(adjacentSides[i]), normal)) < 0.01) {
				uint adjacentGiIndex = GetGiIndex(worldPosition + adjacentSides[i], 1);
				if (abs(GetGi(adjacentGiIndex).frameIndex - int64_t(camera.frameIndex)) < ACCUMULATOR_MAX_FRAME_INDEX_DIFF && GetGi(adjacentGiIndex).radiance.a > 1) {
...
void main() {
	...
	surface.voxelIndex = uint16_t(voxelIndex);
	surface.chunkAddr = AABB.extra;
	surface.color = vec4(vec3(0.5),1);
	surface.normal = BOX_NORMAL_DIRS[normalIndex];
	if (dot(surface.normal, gl_WorldRayDirectionEXT) > 0) {
		surface.normal *= -1;
		surface.ior = 1.0 / surface.ior;
	}
	
	const ivec3 iPos = VoxelIndex_iPos(voxelIndex);
	surface.posInVoxel = localPosition - vec3(voxelData.aabbOffset + iPos) * voxelData.voxelSize;
	...
	if ((camera.options & RENDER_OPTION_ACCUMULATE) != 0) {
		// Path Tracing
	...
	if (RAY_RECURSIONS < RAY_MAX_RECURSION) {
		...
		if (!rayIsGi) {
			...
			if (thisSurface.metallic + thisSurface.specular > 0) {
				...
				vec3 reflectDir = normalize(reflect(gl_WorldRayDirectionEXT, thisSurface.normal));
				RAY_RECURSION_PUSH
					traceRayEXT(tlas, 0, RENDERABLE_STANDARD, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, worldPosition, cam.zNear, reflectDir, cam.zFar, 0);
				RAY_RECURSION_POP
	...
	if (recursions == 0) {
		// Aim Wireframe
		if (renderer.aim.aimID == gl_InstanceCustomIndexEXT && renderer.aim.geometryIndex == gl_GeometryIndexEXT && renderer.aim.aabbIndex == gl_PrimitiveID) {
		...
		if (camera.debugViewMode == RENDERER_DEBUG_MODE_UVS) {
			imageStore(img_debug, imgCoords, vec4(thisSurface.uv, 0, 1));
		}
	}
...
#line 813 "/home/olivier/projects/chill/src/v4d/modules/V4D_rays/assets/shaders/raytracing.glsl"
