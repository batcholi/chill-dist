...
#define VOXEL_INDEX_BITS_XZ 3 // 3=8x8x1024, 4=16x16x256, 5=32x32x64

#define VOXEL_INDEX_BITS_Y uint16_t(16u - VOXEL_INDEX_BITS_XZ - VOXEL_INDEX_BITS_XZ)
// VOXELS_* is the TOTAL NUMBER of voxels in that dimension
#define VOXELS_XZ uint16_t(1u << VOXEL_INDEX_BITS_XZ)
#define VOXELS_Y uint16_t(1u << VOXEL_INDEX_BITS_Y)
#define VOXELS_X VOXELS_XZ
#define VOXELS_Z VOXELS_XZ
#define VOXELS_PER_CHUNK (uint32_t(VOXELS_X)*VOXELS_Y*VOXELS_Z)

// up to 8 flags (for now)
#define VOXEL_FLAG_OPAQUE (1u<<0)

#ifdef __cplusplus // C++
	union VoxelIndex {
		uint16_t index;
		struct Position {
			uint16_t x : VOXEL_INDEX_BITS_XZ;
			uint16_t z : VOXEL_INDEX_BITS_XZ;
			uint16_t y : VOXEL_INDEX_BITS_Y;
			Position(uint16_t x, uint16_t y, uint16_t z) : x(x), z(z), y(y) {}
		} pos;
		VoxelIndex(uint16_t index = 0) : index(index) {
			assert(index < VOXELS_PER_CHUNK);
		}
		VoxelIndex(const glm::ivec3& p) : pos(p.x, p.y, p.z) {
			assert(p.x >= 0);
			assert(p.z >= 0);
			assert(p.y >= 0);
			assert(p.x < VOXELS_X);
		...
		VoxelIndex operator + (const glm::ivec3& offset) const {
			glm::ivec3 p = (*this) + offset;
			return VoxelIndex{p};
		}
	};
	STATIC_ASSERT_SIZE(VoxelIndex, 2);
#else // GLSL
	#define VoxelIndex(x,y,z) (uint16_t(x) | (uint16_t(z) << VOXEL_INDEX_BITS_XZ) | (uint16_t(y) << (VOXEL_INDEX_BITS_XZ+VOXEL_INDEX_BITS_XZ)))
#endif
...
STATIC_ASSERT_ALIGNED16_SIZE(ChunkLightingData, 16*VOXELS_PER_CHUNK + 4*VOXELS_PER_CHUNK);
...
BUFFER_REFERENCE_STRUCT_READONLY(16) ChunkData {
	...
	aligned_VkDeviceAddress extra;
	// blocks
	uint16_t type[VOXELS_PER_CHUNK];
	uint64_t data[VOXELS_PER_CHUNK];
	uint8_t flags[VOXELS_PER_CHUNK];
};
STATIC_ASSERT_ALIGNED16_SIZE(ChunkData, 48 + (2+8+1)*VOXELS_PER_CHUNK);

#ifndef __cplusplus // GLSL
	bool GetVoxel(inout uint64_t chunkID, inout ivec3 pos) {
		...
		while (pos.x < 0 && chunk.minusX != 0) {
			chunk = ChunkData(chunk.minusX);
			pos.x += VOXELS_X;
		}
		while (pos.z < 0 && chunk.minusZ != 0) {
			chunk = ChunkData(chunk.minusZ);
			pos.z += VOXELS_Z;
		}
		while (pos.x >= VOXELS_X && chunk.plusX != 0) {
			chunk = ChunkData(chunk.plusX);
			pos.x -= VOXELS_X;
		}
		while (pos.z >= VOXELS_Z && chunk.plusZ != 0) {
			chunk = ChunkData(chunk.plusZ);
			pos.z -= VOXELS_Z;
		}
		chunkID = uint64_t(chunk); // Because of the AMD bug mentioned above...
		return pos.x >= 0 && pos.y >= 0 && pos.z >= 0 && pos.x < VOXELS_X && pos.y < VOXELS_Y && pos.z < VOXELS_Z;
	}
	uint64_t GetVoxelData(inout uint64_t chunkID, inout ivec3 pos) {
		if (GetVoxel(chunkID, pos)) {
			return ChunkData(chunkID).data[VoxelIndex(pos.x, pos.y, pos.z)];
		}
		return uint64_t(0); // invalid
	}
	#if defined(SHADER_RCHIT) || defined(SHADER_RAHIT) || defined(SHADER_RINT)
		uint64_t GetVoxelData() {
			ivec3 pos = AABB_CENTER_INT;
			if (pos.x >= 0 && pos.y >= 0 && pos.z >= 0 && pos.x < VOXELS_X && pos.y < VOXELS_Y && pos.z < VOXELS_Z) {
				return ChunkData(VOXEL.extra).data[VoxelIndex(pos.x, pos.y, pos.z)];
			}
			return uint64_t(0); // invalid
		}
	#endif
	vec3 GetVoxelLighting(inout uint64_t chunkID, inout ivec3 pos) {
		if (GetVoxel(chunkID, pos)) {
			int voxelIndex = VoxelIndex(pos.x, pos.y, pos.z);
			bool isOpaque = (ChunkData(chunkID).flags[voxelIndex] & VOXEL_FLAG_OPAQUE) != 0;
			if (OPTION_INDIRECT_LIGHTING) {
				return ChunkData(chunkID).lighting.radiance[voxelIndex].rgb * (!isOpaque?1:0);
			} else {
				return vec3(!isOpaque?1:0);
			}
		}
		return vec3(0);
	}
...
const vec3 VOXEL_GRID_SIZE = vec3(1024);
const int VOXEL_GRID_SIZE_HD = 1024;
	...
	CLOSEST_HIT_BEGIN
		...
		if (OPTION_TEXTURES) {
			...
			switch (gl_HitKindEXT) {
				case 0 : BOX_COORD = vec2(ray.localPosition.zy) * vec2(+1,-1) + vec2(BLOCK_GRID_OFFSET);break;
				case 1 : BOX_COORD = vec2(ray.localPosition.xz) * vec2(-1,-1) + vec2(BLOCK_GRID_OFFSET);break;
				case 2 : BOX_COORD = vec2(ray.localPosition.xy) * vec2(-1,-1) + vec2(BLOCK_GRID_OFFSET);break;
				case 3 : BOX_COORD = vec2(ray.localPosition.zy) * vec2(-1,-1) + vec2(BLOCK_GRID_OFFSET);break;
				case 4 : BOX_COORD = vec2(ray.localPosition.xz) * vec2(+1,+1) + vec2(BLOCK_GRID_OFFSET);break;
				case 5 : BOX_COORD = vec2(ray.localPosition.xy) * vec2(+1,-1) + vec2(BLOCK_GRID_OFFSET);break;
			}
