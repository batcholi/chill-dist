...
vec3 GetSkyColor(in vec3 dir) {
	return renderer.skyLightColor * mix(vec3(0.7,0.8,1.5), abs(dir), 0.2) * 2;
}
...
#line 2 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/water.glsl"

#define WATER_INTERSECTION_UNDER 0
#define WATER_INTERSECTION_ABOVE 1


#line 20 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/water.glsl"
#line 1 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/../../../V4D_rtcubes/noise.glsl"
...
#line 21 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/water.glsl"
...
#define WATER_COLOR (vec3(0.005,0.015,0.02) * renderer.skyLightColor)
...
void ApplyUnderwaterFog() {
	...
	const float distFactor = clamp(ray.hitDistance / MAX_WATER_DEPTH, 0 ,1);
...
void main() {
	...
	CLOSEST_HIT_BEGIN
		...
		if ((RT_PAYLOAD_FLAGS & RT_PAYLOAD_FLAG_FOG_RAY) != 0) {
			return;
		}
		...
		ray.color = vec4(vec3(0), WATER_OPACITY);
		...
		if (gl_HitKindEXT == WATER_INTERSECTION_ABOVE) {
			...
			ApplyFresnelReflection(IndexOfRefraction);
			
			// See through water
			if (OPTION_TRANSPARENCY && isFirstRay) {
				...
				if (Refract(rayDirection, waterRay.normal, IndexOfRefraction)) {
					// Launch ray to underwater objects (including water blocks)
					ray.hitDistance = 0;
					traceRayEXT(tlas, 0, RENDERABLE_PRIMARY, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, worldPosition, camera.zNear, rayDirection, camera.zFar, RAY_PAYLOAD_PRIMARY);
					waterRay.color.rgb = ray.color.rgb;
					ray = waterRay;
				}
			}
			
			ray.color.a = 0; // This makes it both transparent and reflective (results in a full untinted reflection added on top of the surface)
	
			UNSET_RT_PAYLOAD_FLAG(RT_PAYLOAD_FLAG_UNDERWATER)
			
		} else {
			SET_RT_PAYLOAD_FLAG(RT_PAYLOAD_FLAG_UNDERWATER)
			...
			if (dot(gl_WorldRayDirectionEXT, vec3(0,1,0)) > 0) {
				...
				float underwaterDepth = waterLevel - ray.localPosition.y;
				float distanceToSurface = clamp(min(underwaterDepth/max(0.001, dot(gl_WorldRayDirectionEXT, vec3(0,1,0))), camera.zFar), camera.zNear, camera.zFar);
				ray.normal = vec3(0,-1,0);
				...
				if (isFirstRay) {
					...
					ray.hitDistance = 0;
					traceRayEXT(tlas, 0, RENDERABLE_PRIMARY_EXCEPT_WATER, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, rayPosition, camera.zNear, rayDirection, distanceToSurface, RAY_PAYLOAD_PRIMARY);
					// Surface refraction from underwater
					if (OPTION_TRANSPARENCY) {
						if (ray.hitDistance == -1) {
							...
							ray.hitDistance = 0;
							// if (dot(rayDirection, vec3(0,1,0)) > 0) UNSET_RT_PAYLOAD_FLAG(RT_PAYLOAD_FLAG_UNDERWATER)
					...
					} else if (ray.hitDistance == -1) {
						ray.color.rgb = renderer.skyLightColor * WATER_OPACITY;
						ray.hitDistance = distanceToSurface;
					}
				} else {
					ray.nextPosition = gl_WorldRayOriginEXT + gl_WorldRayDirectionEXT * distanceToSurface;
					float falloff = smoothstep(0, MAX_WATER_DEPTH, distanceToSurface);
					ray.color.a = pow(falloff, 0.05);
				}
