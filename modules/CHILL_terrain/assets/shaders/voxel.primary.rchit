...
#line 1 "/home/olivier/projects/chill/src/v4d/modules/V4D_rtcubes/noise.glsl"
...
#line 11 "/home/olivier/projects/chill/src/v4d/modules/V4D_rtcubes/base.glsl"
...
#line 5 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/voxel.glsl"
	...
	vec3 GetAmbientVoxelLighting(in vec3 albedo, in ivec3 voxelPosInChunk, in vec3 posInVoxel) {
		...
		if (OPTION_INDIRECT_LIGHTING) {
			...
			int maxBounces = min(RAY_MAX_RECURSION, (isUnderWater?3:2));
		...
		if (OPTION_INDIRECT_LIGHTING && !isGiRay) {
			...
			for (int i = 0; i < nbAdjacentSides; ++i) {
				if (abs(dot(vec3(adjacentSides[i]), ray.normal)) < 0.01) {
...
void main() {
	CLOSEST_HIT_BEGIN
		...
		if (ray.bounces == 0) {
			ivec3 facingWorldPos = ivec3(round(ray.nextPosition + surface.normal * 0.5));
			uint giIndex = GetGiIndex(facingWorldPos);
			// DEBUG_TEST(vec4(Heatmap(float(double(giIndex)/renderer.globalIlluminationTableCount)), 1))
			DEBUG_TEST(vec4(Heatmap(float(double(giIndex)/100)), 1))
		}
		...
		if ((flags & RT_PAYLOAD_FLAG_GI_RAY) != 0 || pushConstant.outputIndex == 2) {
			return;
		}
