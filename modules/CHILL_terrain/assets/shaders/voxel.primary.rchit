...
BUFFER_REFERENCE_STRUCT(16) GlobalLightingData {
	aligned_f32vec4 radiance;
	aligned_f32vec3 _unused;
	aligned_int32_t frameIndex;
};
STATIC_ASSERT_ALIGNED16_SIZE(GlobalLightingData, 32);
...
BUFFER_REFERENCE_STRUCT_READONLY(16) ChunkData {
	...
	BUFFER_REFERENCE_ADDR(GlobalLightingData) globalLighting;
	aligned_uint64_t _unused;
};
STATIC_ASSERT_ALIGNED16_SIZE(ChunkData, 80);
	...
	// 			bool isOpaque = chunk.voxels.fill[voxelIndex] > 0;
	// 			if (OPTION_INDIRECT_LIGHTING) {
	// 				return vec3(isOpaque?0:1); //chunk.lighting.radiance[voxelIndex].rgb * (isOpaque?0:1);
	// 			} else {
	// 				return vec3(isOpaque?0:1);
	// 			}
	// 		} else if (uint64_t(chunk.globalLighting) != 0) {
	// 			return vec3(1); //chunk.globalLighting.radiance.rgb;
	// 		} else {
	// 			return vec3(1,0,1); // ERROR
	// 		}
	// 	}
	// 	return vec3(1); // Non-existent
	// }
#endif
#line 5 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/voxel.glsl"
	...
	#define GI_BOUNCES 2 // max 8
	#define GI_SAMPLES 5 // samples to take in different direction, per gi bounce
	#define DI_BOUNCES 3 // number of bounces in which we trace for Direct Lighting
	...
	void ApplyAmbientVoxelLighting(in ivec3 voxelPosInChunk, in vec3 posInVoxel) {
		...
		if (OPTION_INDIRECT_LIGHTING) {
			...
			if (ray.bounces < GI_BOUNCES) {
				
				// For Performance reasons, we're rounding up the lock index to a higher fraction when geometry is further away, but we're still accumulating in the proper hash
				float dist = length((camera.viewMatrix * vec4(facingWorldPos, 1)).xyz);
				int d = int(floor(max(10, dist - 10) / 10));
				uint giIndexLock = GetGiIndex(facingWorldPos/d*d);
				
				uint lock = atomicExchange(GetGi(giIndexLock).lock, 1);
				if (lock == 0) {
					...
					GetGi(giIndex).frameIndex = int64_t(camera.frameIndex);
					vec3 l = GetGi(giIndex).radiance.rgb;
					GetGi(giIndex).radiance = vec4(mix(l, color, 1.0/accumulation), accumulation);
					GetGi(giIndexLock).lock = 0;
					...
					for (int i = 0; i < nbAdjacentSides; ++i) {
						...
						if (dot(vec3(adjacentSides[i]), ray.normal) == 0) {
							...
							if (lock == 0) {
								...
								GetGi(adjacentGiIndex).radiance = vec4(mix(l, color, mixRatio / accumulation), accumulation);
				...
				if (dot(vec3(adjacentSides[i]), ray.normal) == 0) {
					...
					if (abs(adjacentSides[i].x) + abs(adjacentSides[i].y) + abs(adjacentSides[i].z) == 2) {
						...
						if (adjacentSides[i].x != 0) {
							...
							if (GetVoxel(adjacentVoxelChunkID, pos) && uint64_t(ChunkData(adjacentVoxelChunkID).voxels) != 0 && ChunkData(adjacentVoxelChunkID).voxels.fill[VoxelIndex(pos.x, pos.y, pos.z)] == VOXEL_FULL) {
								diagonalsOccluded++;
							}
						}
...
void main() {
	CLOSEST_HIT_BEGIN
		...
		switch (int(normalIndex)) {
			case 0 : BOX_COORD = vec2(posInVoxel.zy) * vec2(+1,-1) - vec2(VOXEL_GRID_OFFSET); break;
			case 1 : BOX_COORD = vec2(posInVoxel.xz) * vec2(-1,-1) - vec2(VOXEL_GRID_OFFSET); break;
			case 2 : BOX_COORD = vec2(posInVoxel.xy) * vec2(-1,-1) - vec2(VOXEL_GRID_OFFSET); break;
			case 3 : BOX_COORD = vec2(posInVoxel.zy) * vec2(-1,-1) - vec2(VOXEL_GRID_OFFSET); break;
			case 4 : BOX_COORD = vec2(posInVoxel.xz) * vec2(+1,+1) - vec2(VOXEL_GRID_OFFSET); break;
			case 5 : BOX_COORD = vec2(posInVoxel.xy) * vec2(+1,-1) - vec2(VOXEL_GRID_OFFSET); break;
		}
