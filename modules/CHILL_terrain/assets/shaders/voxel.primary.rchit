	...
	vec3 GetAmbientVoxelLighting(in vec3 albedo, in ivec3 voxelPosInChunk, in vec3 posInVoxel) {
		...
		if (OPTION_INDIRECT_LIGHTING) {
			...
			if (ray.bounces < maxBounces) {
				...
				if (!isGiRay && !isFogRay && !isUnderWater) {// Set Current Voxel GI Darker if opaque
					if (IsVoxel(chunkID, voxelPosInChunk) && ChunkData(chunkID).voxels.fill[VoxelIndex(voxelPosInChunk.x, voxelPosInChunk.y, voxelPosInChunk.z)] == VOXEL_FULL) {
						...
						if (lock == 0) {
							...
							GetGi(giIndexCurrent).radiance = vec4(mix(l, vec3(0), min(1, 16.0 / accumulation)), accumulation);
				...
				if (lock == 0) {
					...
					for (int i = 0; i < sampleCount; ++i) {
						...
						ray.hitDistance = 0;
						traceRayEXT(tlas, 0, RENDERABLE_PRIMARY_EXCEPT_WATER, 0/*rayType*/, SBT_HITGROUPS_PER_GEOMETRY/*nbRayTypes*/, 0/*missIndex*/, originalRay.nextPosition, camera.zNear, randomBounceDirection, camera.zFar, RAY_PAYLOAD_PRIMARY);
		...
		if (OPTION_INDIRECT_LIGHTING && !isGiRay && !isFogRay) {
			ivec3 facingVoxelPos = voxelPosInChunk + ivec3(round(ray.normal));
			vec4 lighting;
			if (GetGi(giIndex).iteration == renderer.giIteration && abs(GetGi(giIndex).frameIndex - int64_t(camera.frameIndex)) < ACCUMULATOR_MAX_FRAME_INDEX_DIFF) {
...
#line 373 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/voxel.glsl"
...
void main() {
	CLOSEST_HIT_BEGIN
		...
		// Lighting and Albedo
		ray.color.rgb = mix(GetAmbientVoxelLighting(surface.color.rgb, iPos, surface.posInVoxel) + GetDirectLighting(surface.color.rgb, surface.diffuse, surface.specular, 10/*specularPower*/), surface.color.rgb, clamp(surface.metallic, 0, 1));
