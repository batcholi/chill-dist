...
#line 5 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/voxel.glsl"
	...
	vec3 ApplyAmbientVoxelLighting(in vec3 albedo, in ivec3 voxelPosInChunk, in vec3 posInVoxel) {
		...
		if (OPTION_INDIRECT_LIGHTING) {
			...
			int maxBounces = dist > GI_2_BOUNCES_MAX_DISTANCE? 1:2;
			
			if (ray.bounces < maxBounces) {
				
				if (ray.bounces == 0) {// Set Current Voxel GI Darker if opaque
					if (IsVoxel(chunkID, voxelPosInChunk) && ChunkData(chunkID).voxels.fill[VoxelIndex(voxelPosInChunk.x, voxelPosInChunk.y, voxelPosInChunk.z)] == VOXEL_FULL) {
						...
						if (lock == 0) {
							...
							if (abs(GetGi(giIndexCurrent).frameIndex - int64_t(camera.frameIndex)) > ACCUMULATOR_MAX_FRAME_INDEX_DIFF) {
								accumulation = 1;
							}
							GetGi(giIndexCurrent).frameIndex = int64_t(camera.frameIndex);
							vec3 l = GetGi(giIndexCurrent).radiance.rgb;
							GetGi(giIndexCurrent).radiance = vec4(mix(l, vec3(0), min(1, 100.0 / accumulation)), accumulation);
				...
				if (lock == 0) {
					...
					float accumulation;
					if (ray.bounces == 0) {
						accumulation = min(GetGi(giIndex).radiance.a + 1, MAX_ACCUMULATION);
					...
					vec3 color = vec3(0);
					int sampleCount = int(round(mix(GI_SAMPLES_MAX, GI_SAMPLES_MIN, smoothstep(GI_SAMPLES_FALLOFF_START_DISTANCE, GI_SAMPLES_FALLOFF_END_DISTANCE, dist))));
						...
						for (int i = 0; i < nbAdjacentSides; ++i) {
							if (abs(dot(vec3(adjacentSides[i]), ray.normal)) < 0.2) {
								...
								if (lock == 0) {
									...
									GetGi(adjacentGiIndex).radiance = vec4(mix(l, color, adjacentMixRatio / accumulation), accumulation);
		...
		if (OPTION_INDIRECT_LIGHTING && ray.bounces == 0) {
			ivec3 facingVoxelPos = voxelPosInChunk + ivec3(round(ray.normal));
			vec4 lighting;
			if (abs(GetGi(giIndex).frameIndex - int64_t(camera.frameIndex)) < ACCUMULATOR_MAX_FRAME_INDEX_DIFF) {
...
#line 401 "/home/olivier/projects/chill/src/v4d/modules/CHILL_terrain/assets/shaders/voxel.glsl"
