...
#line 2 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/toneMapping.glsl"


#line 7 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/toneMapping.glsl"

layout(set = 1, input_attachment_index = 0, binding = SET1_BINDING_INOUT_IMG_RESOLVED) uniform subpassInput in_img_resolved;
layout(location = 0) out vec4 out_resolved;

void main() {
	ivec2 coord = ivec2(gl_FragCoord.st);
	vec4 resolved = subpassLoad(in_img_resolved);
	...
	if ((camera.options & RENDER_OPTION_TONE_MAPPING) != 0) {
		...
		resolved.rgb = vec3(1.0) - exp(-resolved.rgb * clamp(exposure, 0.001, 4.0));
	}
	
	// Contrast / Brightness
	if (camera.contrast != 1.0 || camera.brightness != 1.0) {
		resolved.rgb = mix(vec3(0.5), resolved.rgb, camera.contrast) * camera.brightness;
	}
	
	// Gamma correction
	resolved.rgb = pow(resolved.rgb, vec3(1.0 / camera.gamma));
	
	// Output clamped color
	out_resolved = vec4(clamp(resolved.rgb, vec3(0), vec3(1)), 0);
}
