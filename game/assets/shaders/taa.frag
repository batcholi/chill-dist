...
#line 2 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/taa.glsl"


#line 7 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/taa.glsl"

layout(location = 0) out vec4 out_resolved;

void main() {
	vec2 uv = GetFragUV();
	if ((camera.options & RENDER_OPTION_TAA) != 0 && (camera.options & RENDER_OPTION_TEMPORAL_UPSCALING) == 0 && camera.frameIndex > 2) {
		...
		if (ReprojectHistoryUV(uvHistory)) {
			...
			if ((camera.options & RENDER_OPTION_ACCUMULATE) != 0) {
				...
				accumulation = min(history.a, TAA_SAMPLES) + 1;
			}
