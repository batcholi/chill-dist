...
#line 2 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/post.glsl"


#line 7 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/post.glsl"

layout(location = 0) out vec4 out_swapchain;

void main() {
	vec2 uv = GetFragUV() * camera.renderScale;
	out_swapchain = vec4(texture(sampler_post, uv).rgb, 1);
	
	if (camera.debugViewMode != 0) {
		vec4 debugView = imageLoad(img_debug, ivec2(gl_FragCoord.st * camera.renderScale));
		out_swapchain = vec4(mix(out_swapchain.rgb, debugView.rgb, debugView.a), 1);
	}
}
