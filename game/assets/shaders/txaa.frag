...
#line 7 "/home/olivier/projects/chill/src/v4d/game/assets/shaders/txaa.glsl"

layout(location = 0) out vec4 out_resolved;

void main() {
	vec2 uv = GetFragUV();
	if ((camera.options & RENDER_OPTION_TXAA) != 0 && (camera.options & RENDER_OPTION_DLSS) == 0 && camera.frameIndex > 2) {
		vec4 composite = texture(sampler_composite, uv);
		vec2 uvHistory = uv;
		if (ReprojectHistoryUV(uvHistory)) {
			...
			float accumulation = min(history.a, TXAA_SAMPLES) + 1;
			float blend = clamp(1.0 / accumulation, 0, 1);
			out_resolved = vec4(mix(history.rgb, composite.rgb, blend), accumulation);
		} else {
			out_resolved = vec4(composite.rgb, 0);
		}
	} else {
		out_resolved = vec4(texture(sampler_composite, uv).rgb, 0);
	}
}
